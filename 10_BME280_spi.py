...
def writeRegs(regDataList):
    for i in range(0, len(regDataList),2):									# address baytına erişim
        regDataList[i] &= ~(0x80)											# address baytının msb ini 0 yapma
    pi.spi_xfer(h,regDataList)												# spi -> [R/W + address, data, R/W + address, data]
 

def readRegs(regAddr, cnt):
    (_, data) = pi.spi_xfer(h, [regAddr | 0x80] + [0] * cnt)              # msb bit 1 for reading 0 for writing  -  alınacak bilgi büyüklüğünde dizi boyutu belirle
    return list(data[1:])
...
print("ID={}".format(readRegs(IDR_ADDR, 1)))								# değer dönüyorsa bilgi alışverişine hazır demek
...
if dig_H4 > 2047:
    dig_H4 -= 4096
dig_H5 = (calib[31] >> 4) | (calib[32] << 4)
if dig_H5 > 2047:
    dig_H5 -= 4096
dig_H6 = make_S8(calib[33])
